...
import { request, download, logout, API_BASE } from './api/client';
import './styles.css';
...
function Admin() {
  const emptyLab = { slug: '', title: '', category: '', difficulty: 'Basic', description: '', docker_image: '', sandbox_runtime: 'runsc', points: 60, is_active: true };
  const [sessions, setSessions] = useState([]); const [logs, setLogs] = useState([]); const [events, setEvents] = useState([]); const [honeypot, setHoneypot] = useState([]); const [blockedIps, setBlockedIps] = useState([]); const [alerts, setAlerts] = useState([]); const [report, setReport] = useState(null); const [users, setUsers] = useState([]); const [labs, setLabs] = useState([]); const [orchestrator, setOrchestrator] = useState(null);
  const [labForm, setLabForm] = useState(emptyLab); const [adminMsg, setAdminMsg] = useState('');
  async function load() { setSessions(await request('/admin/sessions')); setLogs(await request('/admin/audit')); setEvents(await request('/admin/anti-cheat')); setHoneypot(await request('/admin/honeypot/events')); setBlockedIps(await request('/admin/blocked-ips')); setAlerts(await request('/admin/alerts')); setReport(await request('/admin/reports/security-summary')); setUsers(await request('/admin/users')); setLabs(await request('/admin/labs')); setOrchestrator(await request('/admin/orchestrator/status')); }
  useEffect(() => { load(); }, []);
  async function cleanup() { await request('/admin/cleanup-expired', { method: 'POST' }); load(); }
  async function stopSession(session) {
    if (!confirm(`Stop ${session.container_name}?`)) return;
    setAdminMsg('');
    try {
      const result = await request(`/admin/sessions/${session.id}/stop`, { method: 'POST' });
      setAdminMsg(result.message || 'Session stopped');
      load();
    } catch (e) { setAdminMsg(e.message); }
  }
  async function readAlert(alert) { await request(`/admin/alerts/${alert.id}/read`, { method: 'POST' }); load(); }
  async function readAllAlerts() { await request('/admin/alerts/read-all', { method: 'POST' }); load(); }
  async function createLab() {
    setAdminMsg('');
    try {
      const payload = {
  ...
  return <main><h1><Activity/> Admin Control</h1><button className="primary" onClick={cleanup}>Cleanup expired labs</button><h2>Security Report</h2>{report && <div className="grid"><div className="card"><ShieldAlert/><b>24h Honeypot</b><p>{report.window_24h.honeypot_hits} hits | {report.window_24h.high_severity_honeypot_hits} high severity</p></div><div className="card"><Lock/><b>Auth Signals</b><p>{report.window_24h.failed_logins} failed logins | {report.window_24h.audit_events} audit events</p></div><div className="card"><Server/><b>Operations</b><p>{report.window_24h.active_lab_sessions} active sessions | {report.window_24h.blocked_ips} blocked IPs</p></div><div className="card"><ShieldAlert/><b>Alerts</b><p>{report.window_24h.unread_alerts} unread | {report.window_24h.high_or_critical_alerts} high or critical</p></div></div>}<div className="card exportBar"><button onClick={() => download('/admin/exports/honeypot.csv', 'brovanarange-honeypot.csv')}>Export Honeypot CSV</button><button onClick={() => download('/admin/exports/audit.csv', 'brovanarange-audit.csv')}>Export Audit CSV</button><button onClick={() => download('/admin/exports/anti-cheat.csv', 'brovanarange-anti-cheat.csv')}>Export Anti-Cheat CSV</button><button onClick={() => download('/admin/exports/blocked-ips.csv', 'brovanarange-blocked-ips.csv')}>Export Blocked IPs CSV</button></div><h2>Alert Center</h2><button onClick={readAllAlerts}>Mark all read</button><table><tbody>{alerts.map(a => <tr key={a.id}><td>{new Date(a.created_at).toLocaleString()}</td><td>{a.status}</td><td>{a.severity}</td><td>{a.source}</td><td>{a.title}</td><td>{a.target}</td><td>{a.status === 'unread' && <button onClick={() => readAlert(a)}>Mark read</button>}</td></tr>)}</tbody></table><h2>Sandbox Orchestrator</h2>{orchestrator && <div className="grid"><div className="card"><Server/><b>{orchestrator.sandbox_enforced ? 'Sandbox enforced' : 'Sandbox warning'}</b><p>Configured runtime: {orchestrator.configured_lab_runtime}</p><p>Docker default: {orchestrator.default_runtime}</p><p>Available: {orchestrator.available_runtimes.join(', ')}</p></div></div>}<h2>Manage Labs</h2><div className="card adminForm"><input placeholder="slug" value={labForm.slug} onChange={e => setLabForm({...labForm, slug: e.target.value})}/><input placeholder="title" value={labForm.title} onChange={e => setLabForm({...labForm, title: e.target.value})}/><input placeholder="category" value={labForm.category} onChange={e => setLabForm({...labForm, category: e.target.value})}/><input placeholder="difficulty" value={labForm.difficulty} onChange={e => setLabForm({...labForm, difficulty: e.target.value})}/><input placeholder="docker image, e.g. redrange/my-lab:latest" value={labForm.docker_image} onChange={e => setLabForm({...labForm, docker_image: e.target.value})}/><select value={labForm.sandbox_runtime} onChange={e => setLabForm({...labForm, sandbox_runtime: e.target.value})}><option value="runsc">gVisor runsc</option><option value="runc">Docker runc</option></select><input type="number" placeholder="points" value={labForm.points} onChange={e => setLabForm({...labForm, points: e.target.value})}/><input className="wide" placeholder="description" value={labForm.description} onChange={e => setLabForm({...labForm, description: e.target.value})}/><button className="primary" onClick={createLab}><Plus size={16}/>Add Lab</button>{adminMsg && <b>{adminMsg}</b>}</div><div className="grid">{labs.map(l => <div className="card lab" key={l.id}><h3>{l.title}</h3><p>{l.description}</p><span className="tag">{l.slug}</span><b>{l.points} points | {l.is_active ? 'active' : 'disabled'} | {l.sandbox_runtime}</b><button onClick={() => toggleLab(l)}><Power size={16}/>{l.is_active ? 'Disable' : 'Enable'}</button><button onClick={() => deleteLab(l)}><Trash2 size={16}/>Delete</button></div>)}</div><h2>Active Sessions</h2><div className="grid">{sessions.map(s => <div className="card" key={s.id}><Server/><b>{s.container_name}</b><p>User #{s.user_id} | Lab #{s.lab_id}</p><p>Runtime: {s.container?.runtime || 'unknown'} | RAM: {s.container?.memory_bytes || 0} bytes | PIDs: {s.container?.pids || 0}</p><button onClick={() => stopSession(s)}><Power size={16}/>Stop Session</button></div>)}</div><h2><ShieldAlert/> Honeypot Events</h2><table><tbody>{honeypot.map(h => <tr key={h.id}><td>{new Date(h.last_seen_at).toLocaleString()}</td><td>{h.severity}</td><td>{h.source_ip}</td><td>{h.method} {h.path}</td><td>{h.reputation?.blocked ? 'block recommended' : h.reputation?.error || 'clear'}</td><td>VT malicious: {h.reputation?.malicious ?? 0}</td></tr>)}</tbody></table><h2>Blocked IP Watchlist</h2><table><tbody>{blockedIps.map(ip => <tr key={ip.id}><td>{ip.ip}</td><td>{ip.reason}</td><td>VT malicious: {ip.malicious}</td><td>{ip.country || 'unknown'}</td><td>{ip.as_owner || 'unknown AS'}</td><td>{new Date(ip.last_seen_at).toLocaleString()}</td></tr>)}</tbody></table><h2>Users</h2><table><tbody>{users.map(u => <tr key={u.id}><td>{u.username}</td><td>{u.email}</td><td>{u.role}</td><td>{u.email_verified ? 'verified' : 'unverified'}</td><td>{u.is_active ? 'active' : 'disabled'}</td></tr>)}</tbody></table><h2>Anti-Cheat</h2><table><tbody>{events.map(e => <tr key={e.id}><td>{new Date(e.created_at).toLocaleString()}</td><td>{e.severity}</td><td>{e.reason}</td><td>User #{e.user_id}</td><td>{e.detail}</td></tr>)}</tbody></table><h2>Audit Logs</h2><table><tbody>{logs.map(l => <tr key={l.id}><td>{new Date(l.created_at).toLocaleString()}</td><td>{l.action}</td><td>{l.target}</td><td>{l.detail}</td></tr>)}</tbody></table></main>;
}
