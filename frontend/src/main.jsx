...
function Admin() {
  const emptyLab = { slug: '', title: '', category: '', difficulty: 'Basic', description: '', docker_image: '', sandbox_runtime: 'runsc', points: 60, is_active: true };
  const [sessions, setSessions] = useState([]); const [logs, setLogs] = useState([]); const [events, setEvents] = useState([]); const [honeypot, setHoneypot] = useState([]); const [blockedIps, setBlockedIps] = useState([]); const [users, setUsers] = useState([]); const [labs, setLabs] = useState([]); const [orchestrator, setOrchestrator] = useState(null);
  const [labForm, setLabForm] = useState(emptyLab); const [adminMsg, setAdminMsg] = useState('');
  async function load() { setSessions(await request('/admin/sessions')); setLogs(await request('/admin/audit')); setEvents(await request('/admin/anti-cheat')); setHoneypot(await request('/admin/honeypot/events')); setBlockedIps(await request('/admin/blocked-ips')); setUsers(await request('/admin/users')); setLabs(await request('/admin/labs')); setOrchestrator(await request('/admin/orchestrator/status')); }
  useEffect(() => { load(); }, []);
