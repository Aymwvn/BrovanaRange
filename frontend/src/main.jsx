...
import { Shield, FlaskConical, Trophy, User, Terminal as TerminalIcon, LogOut, Activity, Lock, Server, Plus, Trash2, Power } from 'lucide-react';
...
function Admin() {
  const emptyLab = { slug: '', title: '', category: '', difficulty: 'Basic', description: '', docker_image: '', sandbox_runtime: 'runsc', points: 60, is_active: true };
  const [sessions, setSessions] = useState([]); const [logs, setLogs] = useState([]); const [events, setEvents] = useState([]); const [users, setUsers] = useState([]); const [labs, setLabs] = useState([]); const [orchestrator, setOrchestrator] = useState(null);
  const [labForm, setLabForm] = useState(emptyLab); const [adminMsg, setAdminMsg] = useState('');
  async function load() { setSessions(await request('/admin/sessions')); setLogs(await request('/admin/audit')); setEvents(await request('/admin/anti-cheat')); setUsers(await request('/admin/users')); setLabs(await request('/admin/labs')); setOrchestrator(await request('/admin/orchestrator/status')); }
  useEffect(() => { load(); }, []);
